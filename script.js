...
function myMove() {
    var elemBall    = document.getElementById('myBall'); /* получаем ссылку на объект мяч */
    var elemOut     = document.getElementById('out'); /* получаем ссыку на объект вывода счета и время игры */
    ...
    if (idInterval > 0) {return} /* если функцию setInterval уже вызывали то выходим из скрипта */
    idInterval  = setInterval(frame, 10); /*  позволяет регулярно вызывать указанную функцию через указанный промежуток времени в миллесекундах. */
    ...
    function frame() {
        ...
        if  (posX >= 330 && randomX > 0 && /* координаты мяча 'posY' должен попасть между размерами ракетки -> 'posArrow' */
            (posArrow <=  (posY + 30 ) && ( posY + 15) <=  (posArrow + 100))) {
            ...
            if ((posCount % 5) == 0)  { speed ++;  }

        } else
        /* проверка координат мяча при выходе за пределы игрового поля */
        if (posX < 0 || posY < 0 || posX > 350 || posY > 350 ) {

            /* попадание в нижнуюю границу */    
            if (posX > 350) { /* обрабатываем проигрыш (мячь прошел мимо ракетки) */ 
                /* меняем направление мяча */
                randomX = randomX * (-1); 
                ...
                /* этот код остановит игру в момент когда игрок ошибся */
                 myFinish ()  
            }

            /** меняем направление и скорость мяча при столкновение с боковыми и вехними границами (нижняя граница описана выше) */
            if (posX <= 0)  { randomX = getRandomInt(speed, speed + difference);  }
            if (posY <= 0)  { randomY = getRandomInt(speed, speed + difference);  }
            if (posY > 350) { randomY = getRandomInt(speed, speed + difference) * (-1) ; }
        }

        /* вывод информацию о счете и времени игры */
        elemOut.innerHTML = 'отбил: ' +  posCount + ' за: ' +  formatTime (new Date().getTime() - start) + ' сек.';
...
document.addEventListener('keydown', function(event)  
    {
        ...
        if (id_start > 0) {
            ...
            if (event.key == "ArrowRight")  {posArrow = posArrow + 100}
            if (event.key == "ArrowLeft")   {posArrow = posArrow - 100}
            
            if (posArrow < 0 ) {posArrow = 0;}
            if (posArrow > 300 ) {posArrow = 300;}
